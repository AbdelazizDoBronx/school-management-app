...
const getTimeLeftBadge = (deadline) => {
  ...
  if (days > 0) {
    return { text: `${days} day${days > 1 ? 's' : ''} left`, color: "bg-blue-500" };
  }
  if (hours > 0) {
  ...
  return (
    ...
    >
      ...
      <div className="card-body">
        ...
        {/* Time Left Badge */}
        <div className={`badge ${color} mb-4 text-white p-3`}>{text}</div>
        ...
        <div className="flex justify-center">
          <Link
            to={`/quiz/${quiz.id}`}
            className="btn btn-primary w-full text-white  bg-blue-500 hover:bg-blue-700 rounded-lg focus:outline-none transition duration-200 ease-in-out"
          >
